...
class LtInstruction implements IInstruction
{
    ...
    public function __construct(array $args, Interpreter $interpreter)
    {
        ...
        if (count($this->args) !== 3) {
            throw new SourceStructureException("LT instruction has invalid amount of arguments");
        }

        // arg1 check
        if ($this->args[0]['type'] !== 'var') {
            throw new OperandTypeException("LT instruction has invalid type of argument 1");
        }

        // arg2 check
        if ($this->args[1]['type'] !== 'var' && $this->args[1]['type'] !== 'int' && $this->args[1]['type'] !== 'bool' && $this->args[1]['type'] !== 'string' && $this->args[1]['type'] !== 'nil') {
            throw new OperandTypeException("LT instruction has invalid type of argument 2");
        }

        // arg3 check
        if ($this->args[2]['type'] !== 'var' && $this->args[2]['type'] !== 'int' && $this->args[2]['type'] !== 'bool' && $this->args[2]['type'] !== 'string' && $this->args[2]['type'] !== 'nil') {
            throw new OperandTypeException("LT instruction has invalid type of argument 3");
        }
    }
    ...
    {        
        ...
        if(preg_match('/^(GF|LF|TF)@(.+)$/', $this->args[0]['value'], $matches)) {
            ...
            if (!$this->interpreter->isVarDefined($varFrame, $varName)) {
                throw new VariableAccessException("LT instruction has undefined variable");
            }
        } else {
            throw new OperandValueException("LT instruction has invalid format of argument 1");
        }

        // retrieve the value of symb1
        $symb1Value = $this->getSymbValue($this->args[1]);

        // retrieve the value of symb2
        $symb2Value = $this->getSymbValue($this->args[2]);

        if ($symb1Value['type'] !== $symb2Value['type']) {
            throw new OperandTypeException("LT instruction has different operand types");
        }
        ...
        switch ($symb1Value['type']) {
            ...
            case 'bool':
                ...
                $result = $bool1 < $bool2 ? 'true' : 'false'; // false < true
                break;
            case 'string':
                $result = strcmp($symb1Value['value'], $symb2Value['value']) < 0 ? 'true' : 'false';
                break;
            default:
                throw new OperandTypeException("LT instruction has nil as a type of argument");
        }
        $type = 'bool';
    ...
    private function getSymbValue($symbol): array {
        if ($symbol['type'] === 'var') {
            ...
            if(preg_match('/^(GF|LF|TF)@(.+)$/', $symbol['value'], $matches)) {
                ...
                throw new OperandValueException("LT instruction has invalid format of argument");
            }
            ...
            throw new OperandTypeException("LT instruction has invalid type of argument");
        }
    }
}