...
class IdivInstruction implements IInstruction
{
    ...
    public function __construct(array $args, Interpreter $interpreter)
    {
        ...
        if (count($this->args) !== 3) {
            throw new SourceStructureException("IDIV instruction has invalid amount of arguments");
        }

        // arg1 check
        if ($this->args[0]['type'] !== 'var') {
            throw new OperandTypeException("IDIV instruction has invalid type of argument 1");
        }

        // arg2 check
        if ($this->args[1]['type'] !== 'var' && $this->args[1]['type'] !== 'int') {
            throw new OperandTypeException("IDIV instruction has invalid type of argument 2");
        }

        // arg3 check
        if ($this->args[2]['type'] !== 'var' && $this->args[2]['type'] !== 'int') {
            throw new OperandTypeException("IDIV instruction has invalid type of argument 3");
        }
    }
    ...
    public function execute(): void
    {
        ...
        if(preg_match('/^(GF|LF|TF)@(.+)$/', $this->args[0]['value'], $matches)) {
            ...
            if (!$this->interpreter->isVarDefined($varFrame, $varName)) {
                throw new VariableAccessException("IDIV instruction has undefined variable");
            }
        } else {
            throw new OperandValueException("IDIV instruction has invalid format of argument 1");
        }
        ...
        if ($symb2Value === 0) {
            throw new OperandValueException("IDIV instruction has division by zero");
        }
    ...
    private function getSymbValue($symbol): int {
        if ($symbol['type'] === 'var') {
            ...
            if(preg_match('/^(GF|LF|TF)@(.+)$/', $symbol['value'], $matches)) {
                ...
                {
                    throw new OperandTypeException("IDIV instruction has invalid type of argument");
                }
                return intval($symbValue['value']);
            }
            else {
                throw new OperandValueException("IDIV instruction has invalid format of argument");
            }
